% plotIInj_AEPPEP_allFlies.m
%
% Function that takes output from extractLegStepParamsIInj_fly() and
...
%   4/27/24 - HHY
%
function plotIInj_AEPPEP_allFlies(datDir, whichParam, whichPhase, ...
    durs, amps, xScale, yScale, plotIndiv, plotDiff)

    % legs to subplot indices
    % puts left legs on left, and front legs on top
    subInd = [2 4 6 1 3 5]; 

    % prompt user to select output files from extractLegStepParamsIInj_fly()
    ...
    for i = 1:numFlies
        ...
        % load data
        load(outputFullPath, 'legStepsIInjMeans', 'condKeyDurs', ...
            'condKeyAmps');
        ...
        if (strcmpi(whichPhase, 'stance'))
            if (strcmpi(whichParam, 'AEP'))
                thisMeanX = legStepsIInjMeans.stance.stepAEPX;
                thisMeanY = legStepsIInjMeans.stance.stepAEPY;
            elseif (strcmpi(whichParam, 'PEP'))
                thisMeanX = legStepsIInjMeans.stance.stepPEPX;
                thisMeanY = legStepsIInjMeans.stance.stepPEPY;
            else
        ...
        elseif (strcmpi(whichPhase, 'swing'))
            if (strcmpi(whichParam, 'AEP'))
                thisMeanX = legStepsIInjMeans.swing.stepAEPX;
                thisMeanY = legStepsIInjMeans.swing.stepAEPY;
            elseif (strcmpi(whichParam, 'PEP'))
                thisMeanX = legStepsIInjMeans.swing.stepPEPX;
                thisMeanY = legStepsIInjMeans.swing.stepPEPY;
            else
                disp('Invalid step parameter. AEP or PEP only.')
                return;
            end
...
%             noStimInd = find(condKeyAmps == -75);
    ...
    % get which conditions to plot (of specified durs and amps)
    plotCondInd = [];
    ...
    for i = 1:length(plotCondInd)
        legendLabels{i} = sprintf('Amp=%.1f, dur=%.1f s', ...
            condKeyAmps(plotCondInd(i)), condKeyDurs(plotCondInd(i)));
    end
