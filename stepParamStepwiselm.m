% stepParamStepwiselm.m
%
% Function that takes in one or more pData files from the same fly
%  (specified through GUI), a list of step parameters to use as predictors,
%  a time delay, and a sample rate and uses the MATLAB stepwiselm to
...
%   lmParams - struct of parameter values for stepwise fitting
%       criterion - criterion to add or remove terms, 'sse', 'aic', 'bic',
%           'rsquared', 'adjrsquared'
%       pEnter - threshold for criterion to add terms, scalar value
%       pRemove - threshold for criterion to remove terms, scalar value
...
%   7/13/22 - HHY
%
function stepParamStepwiselm(paramList, legList, phaseList, ephysVarName, ...
    timeDelay, sampRate, lmParams, savePath)
...
%                         moveNotMove, thisLeg, thisStepParam, thisPhase);
    ...
    mdl = stepwiselm(predMatrix, ephysVar, 'VarNames',allVarNames, ...
        'Criterion',lmParams.criterion, 'PEnter',lmParams.pEnter,...
        'PRemove',lmParams.pRemove, 'Verbose',1);
