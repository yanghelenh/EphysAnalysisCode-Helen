% plotOptomotorOptoFicTracChangePos_allFlies.m
%
...
%  cumulative position/angle between a start time point and an end time 
%  point relative to the optomotor stimulus start. For each fly, option for
%  having the mean of each optogenetic condition subtracted from the mean
%  of the no stimulation condition, and the mean across flies is plotted.
...
%   4/11/24 - HHY
%
function plotOptomotorOptoFicTracChangePos_allFlies(datDir, whichParam, ...
    whichVel, whichNDs, plotTimes, plotDiff, yScale)
    ...
    for i = 1:numFlies
        ...
        startInd = find(plotTimes(1) <= thisTrialTimes, 1, 'first');
        endInd = find(plotTimes(2) >= thisTrialTimes, 1, 'last');
        ...
        noStimMean = mean(fictracOpto(thisVelInd).(whichParam)(endInd, noStimLog) - ...
                fictracOpto(thisVelInd).(whichParam)(startInd, noStimLog));
        ...
        for j = 1:numNDs
            ...
            %  end time points
            diffAllTrials = (fictracOpto(thisVelInd).(whichParam)(endInd, thisNDLog) - ...
                fictracOpto(thisVelInd).(whichParam)(startInd, thisNDLog));
...
%     semAllFlies = std(allFliesMeans,[],1) / sqrt(numFlies);
    ...
    xlabel('ND');
    ylabel(whichParam);
    title(sprintf('%s, Vel=%.1f, change from t=%.2f to t=%.2f',...
        whichParam, whichVel, plotTimes(1), plotTimes(2)));
end