...
%
% Function that extracts the individual leg traces, FicTrac, and leg step
%  parameters in response to optogenetic stimulation of different durations 
%  and intensities (ND filters) for a single fly.
% Filtered for moving/not moving
...
function computeLegStepsFictracOpto_1Fly(NDs, durs, bwStimDur, savePath)
    ...
    % for each variable, struct for each rep, mean, and std error
    % reps as #NDs x #durs x 6 (# legs) x 2 (swing/stance) cell array,
    %  with each element being a matrix of reps x rep length
    % repsOptoTimes as #NDs x #durs x 6 x 2 cell array, with each
    %  element being a vector of # reps length, tracking opto stim start
    %  time corresponding to each rep
    % repsPDataNames as #NDs x #durs x 6 x 2 cell array, with
    %  each element being a cell array of # reps length, tracking the name
    %  of the pData file the rep came from
    % means and stdErrs as same size cell array, each element as vector
    legStepsFieldStruct.reps = cell(length(NDs),length(durs),NUM_LEGS,...
        NUM_PHASES);
    legStepsFieldStruct.repsOptoTimes = cell(length(NDs),length(durs),...
        NUM_LEGS,NUM_PHASES);
    legStepsFieldStruct.repsPDataNames = cell(length(NDs),length(durs),...
        NUM_LEGS,NUM_PHASES);
    legStepsFieldStrct.means = cell(length(NDs),length(durs),...
        NUM_LEGS, NUM_PHASES);
    legStepsFieldStrct.stdErrs = cell(length(NDs),length(durs),...
        NUM_LEGS,NUM_PHASES);

    for i = 1:length(legStepsVarNames)
        thisVarName = legStepsVarNames{i};
        legStepsOpto.(thisVarName) = legStepsFieldStruct;
    end
    ...
    for i = 1:numPDataFiles
        ...
        if (any(contains(pDataVarsNames, 'legTrack')) && ...
            ...
            for j = 1:length(legStepsVarNames)
                thisVarName = legStepsVarNames{j};
                
                % loop through all legs
                for k = 1:NUM_LEGS
                    whichLeg = legIDs.names{k};
                    whichLegInd = legIDs.ind(k);
                    ...
                    for l = 1:NUM_PHASES
                        ...
                        % get reps
                        [legStepsOpto.(thisVarName).reps(:,:,whichLegInd,l),...
                            legStepsOpto.(thisVarName).repsOptoTimes(:,:,whichLegInd,l),...
                            legStepsOpto.(thisVarName).repsPDatNames(:,:,whichLegInd,l),...
                            legStepsDurTs] = extractTrialsOpto(...
                            legStepsOpto.(thisVarName).reps(:,:,whichLegInd,l), ...
                            legStepsOpto.(thisVarName).repsOptoTimes(:,:,whichLegInd,l),...
                            legStepsOpto.(thisVarName).repsPDataNames(:,:,whichLegInd,l),...
                            thisVarVal, legStepSampTime, opto, NDs, durs, ...
                            bwStimDur, pDataName, false);
                    end
                end
            end
            ...
            clear fictracProc opto legTrack legSteps moveNotMove
        end
    end
    ...
    legStepsOpto.durTs = legStepsDurTs;

    % full path to save data
    saveFullPath = [savePath filesep flyName '_avgLegStepsFictracOpto.mat'];

    % save data
    save(saveFullPath, 'fictracOpto','legTrackOpto','legStepsOpto',...
        'NDs','durs','bwStimDur', '-v7.3');
