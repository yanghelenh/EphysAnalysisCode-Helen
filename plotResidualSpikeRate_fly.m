% plotResidualSpikeRate_fly.m
%
% Function to plot output of getResidualSpikeRate_fly() as
...
%   8/14/23 - HHY
%
function plotResidualSpikeRate_fly(datDir, avg, indivFlies, numBins, xRange)
    ...
    for i = 1:numFlies
        ...
        % load variables
        load(fullFilePath, 'residSpikeRate', 'residBehVars1D', ...
            'ephysParam', 'behParams', 'legs', 'tDelay');

        thisLog = (residBehVars1D >= xRange(1) ) & (residBehVars1D <= xRange(2));

        corrcoef(residBehVars1D(thisLog), residSpikeRate(thisLog))


        % loop through all bins, find mean and SEM
        for j = 1:numBins
            % get logical for which samples fall into this bin
            thisBinLog = (residBehVars1D >= binStarts(j)) & ...
                (residBehVars1D < binEnds(j));
            % get ephys values for these samples
            thisEphys = residSpikeRate(thisBinLog);
    ...
    % ephys param label
    yLblStr = sprintf('%s, delay = %d ms', 'Residual spike rate', ...
        tDelay * 1000);
    ylabel(yLblStr);

    xlabel(behParams{2});

    title(sprintf('Residual after %s', behParams{1}));
end