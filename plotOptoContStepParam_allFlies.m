% plotOptoContStepParam_allFlies.m
%
% Function that takes output files from extractContStepParamsOpto_fly() and
...
%   8/6/23 - HHY
%
function plotOptoContStepParam_allFlies(datDir, whichParam, whichDur,...
    whichND, yScale)
    ...
    % prompt user to select output files from extractContStepParamsOpto_fly()
    ...
    for i = 1:numFlies
        ...
        % load data
        load(outputFullPath, 'legStepsContOptoMean', 'legStepsContOptoSEM', ...
            'durs', 'NDs', 'trialTimes');
        ...
        thisMean = legStepsContOptoMean(thisDurInd, thisNDInd).(whichParam);
        thisSEM = legStepsContOptoSEM(thisDurInd, thisNDInd).(whichParam);
